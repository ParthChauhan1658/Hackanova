...
function AppointmentSection({ toast }: { toast: (m: string, t?: ToastType) => void }) {
  ...
  useEffect(() => {
    fetchCalEventTypes().then(res => {
      if (res.status === 'ok' && res.event_types.length > 0) {
        ...
        setSelectedEventTypeUri(res.event_types[0].uri)
      } else if (res.detail) {
        toast(`Calendly: ${res.detail}`, 'error')
      }
    }).catch(() => { /* calendly offline or token not set */ })
  }, [])
  ...
  async function handleBook(e: React.FormEvent) {
    ...
    try {
      const res = await bookAppointment({
        ...
        reason,
        notes,
        event_type_uri: selectedEventTypeUri || undefined,
      })
      if (res.status === 'scheduled' && res.booking_url) {
        setBookingUrl(res.booking_url)
        toast('Calendly booking link created — share it with the patient!', 'success')
      } else {
        toast(`Booking failed: ${res.detail ?? 'unknown error'}`, 'error')
      }
  ...
  return (
    <Card className="p-5">
      <SectionLabel>Book Appointment via Calendly</SectionLabel>
      <p className="text-[10px] text-[#9b8768] mb-5 leading-relaxed">
        Generate a single-use Calendly scheduling link for the patient. The patient picks their own slot on Calendly's hosted page — no slot conflicts possible.
      </p>

      {bookingUrl && (
        <div className="mb-5 space-y-2 px-4 py-3 rounded-xl bg-[#edf7f4] border border-[#1a6b5830]">
          <div className="flex items-center gap-2">
            <svg className="w-5 h-5 text-[#1a6b58] shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
              <path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/>
            </svg>
            <p className="text-sm font-semibold text-[#1a6b58]">Scheduling Link Created</p>
            <button onClick={() => setBookingUrl(null)} className="ml-auto text-[#9b8768] hover:text-[#6b5438]">
          ...
          <p className="text-[10px] text-[#6b5438] break-all font-mono">{bookingUrl}</p>
          <div className="flex gap-2 pt-1">
            <button
              onClick={() => { window.open(bookingUrl, '_blank'); toast('Opened in new tab', 'info') }}
              className="flex-1 px-3 py-2 rounded-xl bg-[#1a6b58] text-white text-xs font-semibold hover:bg-[#155a49] transition-colors"
            >
              Open Calendly Page
            </button>
            <button
              onClick={() => { navigator.clipboard.writeText(bookingUrl); toast('Link copied!', 'success') }}
              className="px-3 py-2 rounded-xl border border-[#1a6b5840] text-[#1a6b58] text-xs font-semibold hover:bg-[#edf7f4] transition-colors"
            >
              Copy Link
            </button>
          </div>
      ...
      {calEventTypes.length > 0 && (
        <div className="mb-5 flex items-center gap-2 px-3 py-2.5 rounded-xl bg-[#edf7f4] border border-[#1a6b5830]">
          ...
          <span className="text-xs text-[#1a6b58] font-semibold">{calEventTypes.length} event type(s) loaded from Calendly</span>
        </div>
      )}
      ...
      <form onSubmit={handleBook} className="space-y-4">
        ...
        {calEventTypes.length > 0 && (
          <div>
            ...
            <Select value={selectedEventTypeUri} onChange={e => setSelectedEventTypeUri(e.target.value)}>
        ...
        <div className="flex items-center justify-between pt-1">
          <p className="text-[10px] text-[#9b8768]">Patient selects their own slot on Calendly</p>
